...
class CorreoEnviosController extends Controller
{
    public function index(Request $request): View
    {
        ...
        // Los KPIs se calculan sobre el mes en curso sin importar los filtros:
        // son el pulso del módulo, no del listado filtrado.
        $delMes = CorreoEnvio::with('destinatarios')
            ->where('estado', EstadoEnvioCorreo::Enviado)
            ->where('enviado_en', '>=', now()->startOfMonth())
            ->get();

        $destinatariosMes = $delMes->flatMap->destinatarios->where('estado', EstadoDestinatarioCorreo::Enviado);
        $alcanzados = $destinatariosMes->count();
        $abiertos = $destinatariosMes->where('aperturas', '>', 0)->count();
