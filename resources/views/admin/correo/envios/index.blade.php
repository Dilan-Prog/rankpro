...
@section('content')
    ...
    <div class="kpi-grid">
        <x-stat-card label="Enviados este mes" value="{{ $kpis['enviados_mes'] }}" sub="{{ $kpis['programados'] }} en cola" icon="fa-paper-plane" color="primary" />
        <x-stat-card label="Destinatarios alcanzados" value="{{ number_format($kpis['alcanzados']) }}" sub="correos entregados al servidor" icon="fa-users" color="teal" />
        <x-stat-card label="Apertura estimada" value="{{ $kpis['apertura'] !== null ? $kpis['apertura'].'%' : '—' }}" sub="{{ $kpis['abiertos'] }} con al menos una apertura" icon="fa-envelope-open" color="blue" />
        <x-stat-card label="Clics" value="{{ number_format($kpis['clics']) }}" sub="en enlaces del correo" icon="fa-arrow-pointer" color="emerald" />
    </div>
